...
class ScaledDotProductAttention(nn.Module):
    ...
    def forward(self, 
        ...
        if not hasattr(self, 'value'):
            return attention_probs if not return_attention_scores else attention_scores
...
class AdditiveAttention(nn.Module):
    ...
    def forward(self, 
        ...
        assert len(feats) == len(self.linear1_f)
        ...
        for inputs, layer in zip(feats, self.linear1_f):
